...
public class JingleToSdp {
    ...
    public static SessionDescription toSdp(JingleIQ iq, SessionDescription.Type type) {
    ...
    private static void appendSCTPLines(ContentPacketExtension cpe, StringBuilder sb) {
        ...
        sb.append("a=setup:actpass").append(NL);//linpeng add
    ...
    private static void appendMLine(ContentPacketExtension cpe, StringBuilder sb) {
        ...
        for (PayloadTypePacketExtension pt : description.getPayloadTypes()) {
            ...
            if (pt.getParameters().size() > 0) {
                sb.append("a=fmtp:126 level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=42e01f").append(NL);
            }
        ...
        for (SourcePacketExtension ssrc
            ...
            for (ParameterPacketExtension param : ssrc.getParameters()) {
                ...
                if (!param.getValue().equals("mixedlabelvideo0")) {
                    if ("label".equals(param.getName()) && "video".equals(cpe.getName())) {
                        Log.i(TAG, "-ssrcAndLabel _ssrc=" + ssrcL + ",label=" + param.getValue());
                        XmppConnection.ssrcAndLabel.put("" + ssrcL, param.getValue());
                    }
                }
            }
    ...
    public static SessionDescription addSSRCs(SessionDescription sdp, MediaSSRCMap ssrcToAdd) {
        ...
        for (int i = 1; i < parts.length; i++) {
            ...
            for (SourcePacketExtension ssrcPe : ssrcToAdd.getSSRCsForMedia(media)) {
                if (mediaPart.contains("a=ssrc:" + ssrcPe.getSSRC())) {//SSRC=科学分析和研究中心
                    // 已经包括SSRC
                    continue;
                }
...
//        if (sdp.description == null || TextUtils.isEmpty(sdp.description)) {
//            return null;
//        }
        String[] parts = sdp.description.split("m=");// sdp频繁空指针异常
        StringBuilder outputSdp = new StringBuilder(parts[0]);
        for (int i = 1; i < parts.length; i++) {
            String mediaPart = parts[i];
            String media = mediaPart.substring(0, mediaPart.indexOf(" "));
